...
@implementation NSDToast

-(instancetype)init{
   return [self initWithMargin:0];
}

-(instancetype)initWithMargin:(int) margin{
    self = [super init];
    if (self) {
        self.margin = margin;
    }
...
-(void)displayOnView:(UIView *)view withMessage:(NSString *)message andColor:(UIColor*)color andIndicator:(BOOL)indicator andFaded:(BOOL)faded
{
    ...
    UIView *messageFrame = [[UIView alloc] initWithFrame:CGRectMake(MESSAGE_FRAME_X, MESSAGE_FRAME_BEGIN_Y+_margin+50, view.frame.size.width, MESSAGE_FRAME_HEIGHT)];
    ...
    [UIView animateWithDuration:ANIMATION_SPEED animations:^{
        messageFrame.frame = CGRectMake(MESSAGE_FRAME_X, MESSAGE_FRAME_END_Y+_margin, view.frame.size.width, MESSAGE_FRAME_HEIGHT);
    }];
...
-(void)removeFromView:(UIView*)view
{
    for(UIView *child in [view subviews])
    {
        ...
        {
            [UIView animateWithDuration:ANIMATION_SPEED animations:^{
                child.frame = CGRectMake(MESSAGE_FRAME_X, MESSAGE_FRAME_BEGIN_Y+_margin+50, view.frame.size.width, MESSAGE_FRAME_HEIGHT);
